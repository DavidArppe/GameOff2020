...
public static class TextureArrayHelpers
{
    ...
    public static void ClearToBlack(RenderTexture dst)
    {
        ...
        s_clearToBlackShader.Dispatch(
            krnl_ClearToBlack,
            TerrainRenderer.Instance.LodDataResolution / LODDataManager.THREAD_GROUP_SIZE_X,
            TerrainRenderer.Instance.LodDataResolution / LODDataManager.THREAD_GROUP_SIZE_Y,
            dst.volumeDepth
        );
    }
    ...
    public static Texture2DArray CreateTexture2DArray(Texture2D texture)
    {
        var array = new Texture2DArray(
            ...
            LODDataManager.MAX_LOD_COUNT,
            texture.format,
            false,
            false
        );

        for (int textureArrayIndex = 0; textureArrayIndex < LODDataManager.MAX_LOD_COUNT; textureArrayIndex++)
        {
