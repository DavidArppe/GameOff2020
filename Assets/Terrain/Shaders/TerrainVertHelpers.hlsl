#ifndef TERRAIN_VERT_HELPERS_H
#define TERRAIN_VERT_HELPERS_H
...
{
	// taxicab distance from terrain center drives LOD transitions
	float2 offsetFromCenter = abs(float2(i_worldPos.x - _TerrainCenterPosWorld.x, i_worldPos.z - _TerrainCenterPosWorld.z));
	...
	// strip width.
	lodAlpha = max((lodAlpha - _TerrainLodAlphaBlackPointFade) / _TerrainLodAlphaBlackPointWhitePointFade, 0.);
...
void SnapAndTransitionVertLayout(in const float i_meshScaleAlpha, in const CascadeParams i_cascadeData0, in const float i_geometryGridSize, inout float3 io_worldPos, out float o_lodAlpha)
{
	...
	// :TerrainGridPrecisionErrors
	const float GRID_SIZE_2 = 2.000001 * i_geometryGridSize;
	const float GRID_SIZE_4 = 2.0 * GRID_SIZE_2;
